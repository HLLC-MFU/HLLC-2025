...
import (
    ...
    // FindAllWithNestedPopulate finds documents and populates nested fields
    func (s *BaseService[T]) FindAllWithNestedPopulate(ctx context.Context, opts QueryOptions, populateConfigs []NestedPopulateConfig) (*Response[T], error) {
        pipeline := []bson.M{
            { "$match": opts.Filter },
        }
