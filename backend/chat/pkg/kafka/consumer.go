...
func NewConsumer(brokers []string, topics []string, groupID string, handler MessageHandler) *Consumer {
...
func (c *Consumer) Start() error {
	for _, topic := range c.topics {
		...
		log.Printf("[Kafka Consumer] Started consuming from topic: %s", topic)
	}
	return nil
}
...
func (c *Consumer) Stop() {
	...
	c.cancel()
	c.wg.Wait()
	log.Printf("[Kafka Consumer] Stopped consuming from topic: %s", c.topics)
}
...
func (c *Consumer) consume() {
	...
	log.Printf("[Kafka Consumer] Started consuming from topic: %s (group: %s)", c.topics, c.groupID)

	for {
		select {
		case <-c.ctx.Done():
			log.Printf("[Kafka Consumer] Shutting down consumer for topic: %s", c.topics)
			return
		default:
			...
			if err != nil {
				...
				case err == context.Canceled:
					log.Printf("[Kafka Consumer] Consumer canceled for topic: %s", c.topics)
					return
				case err.Error() == "EOF" || err.Error() == "connection refused":
					...
					log.Printf("[Kafka Consumer] Reconnected to broker for topic: %s", c.topics)
					continue
				default:
					log.Printf("[Kafka Consumer] Unexpected error reading message from topic %s: %v", c.topics, err)
			...
			if err := json.Unmarshal(msg.Value, &chatMsg); err != nil {
				log.Printf("[Kafka Consumer] Error unmarshaling message from topic %s: %v", c.topics, err)
				continue
			}

			if err := c.handler.HandleMessage(c.ctx, &chatMsg); err != nil {
				log.Printf("[Kafka Consumer] Error handling message from topic %s: %v", c.topics, err)
				continue
			}
