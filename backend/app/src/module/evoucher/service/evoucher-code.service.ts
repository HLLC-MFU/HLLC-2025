...
export class EvoucherCodeService {
  ...
  async findAll(query: Record<string, string>) {
    return await queryAll<EvoucherCode>({
      ...
      populateFields: (excluded) =>
        Promise.resolve(
          excluded.includes('evoucher') ? [] : [{ path: 'evoucher' }],
        ),
    });
  }
  ...
  async findOneByQuery(query: Partial<EvoucherCode>) {
    return queryFindOne<EvoucherCode>(this.evoucherCodeModel, query, [
      { path: 'evoucher' },
      { path: 'user' },
    ]);
  }
  ...
  async getExistsEvoucherCodes(
    userId: string,
  ): Promise<{ evoucher: Evoucher; exists: boolean; available: boolean }[]> {
    const evouchers = await this.evoucherModel.find({}).lean();
  ...
  async update(id: string, updateEvoucherCodeDto: UpdateEvoucherCodeDto) {
    ...
    const voucherCode = await findOrThrow(
      this.evoucherCodeModel,
      id,
      'Voucher code',
    );
    ...
    return await queryUpdateOne<EvoucherCode>(
      this.evoucherCodeModel,
      id,
      updateEvoucherCodeDto,
    );
  }
  ...
  async generateEvoucherCodes(dto: CreateEvoucherCodeDto & { count: number }) {
    ...
    if (new Date() > new Date(evoucher.expiration)) {
      throw new BadRequestException(
        'Cannot generate code for expired evoucher',
      );
    }
