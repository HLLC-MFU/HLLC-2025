...
function generateUserMetadata(
  ...
  if (randomSchool) {
    ...
    if (schoolMajors && schoolMajors.length > 0) {
      randomMajor =
        schoolMajors[Math.floor(Math.random() * schoolMajors.length)];
    }
  }
...
async function seedTestUsers(): Promise<void> {
  ...
  try {
    ...
    console.log(`✅ พบ role "${config.roleName}" ID: ${role._id.toString()}`);
    logOutput += `Role: ${config.roleName} (${role._id.toString()})\n`;
    ...
    for (let batch = 0; batch < totalBatches; batch++) {
      ...
      const batchEndIdx = Math.min(
        batchStartIdx + config.batchSize,
        config.totalUsers,
      );
      const batchSize = batchEndIdx - batchStartIdx;

      console.log(
        `📦 กำลังสร้างชุดที่ ${batch + 1}/${totalBatches} (ผู้ใช้ ${batchStartIdx + 1}-${batchEndIdx})`,
      );
      ...
      for (let i = batchStartIdx; i < batchEndIdx; i++) {
        ...
        users.push({
          ...
          metadata:
            schools.length > 0
              ? generateUserMetadata(schools, majorsBySchool)
              : {},
          createdAt: new Date(),
          updatedAt: new Date(),
        });
      ...
      console.log(
        `✅ สร้างผู้ใช้ในชุดที่ ${batch + 1} จำนวน ${insertResult.insertedCount} คน สำเร็จใน ${batchDuration}ms (เฉลี่ย ${avgTimePerUser.toFixed(2)}ms/คน)`,
      );
    ...
    console.log(
      `⏱️ ใช้เวลาทั้งหมด ${totalDuration}ms (เฉลี่ย ${avgTimePerUser.toFixed(2)}ms/คน)`,
    );
    console.log('');
