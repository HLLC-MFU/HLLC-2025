...
import { useEvoucherClaim } from '@/hooks/chats/useEvoucherClaim';
...
const MessageBubble = memo(({
  ...
  const { claimEvoucher } = useEvoucherClaim();
  ...
  const showDialog = useCallback((message: string, type: 'success' | 'error' | 'info' = 'info') => {
    console.log('🔥 [DEBUG] showDialog called with:', { message, type });
    setClaimDialogMessage(message);
    setClaimDialogType(type);
    setShowClaimDialog(true);
    console.log('🔥 [DEBUG] showClaimDialog set to true');
    // Auto hide after 5 seconds (increased from 3)
    setTimeout(() => {
      console.log('🔥 [DEBUG] Auto-hiding dialog after 5 seconds');
          ...
          {parts.map((part, i) => {
            if (regex.test(part)) {
              ...
              if (isMatch) {
                ...
                return (
                  <span key={i} className={`inline font-semibold px-1.5 py-0.5 rounded-md ${isReplyPreview
                    ? 'text-blue-400'
                    : isMyMessage
              ...
              return (
                <span key={i} className={`inline font-medium px-1 py-0.5 rounded ${isReplyPreview
                  ? 'text-blue-500'
                  : isMyMessage
    ...
    if (message.type === 'evoucher') {
      ...
      if (evoucherInfo && Object.keys(evoucherInfo).length > 0) {
        ...
        const handleClaim = async () => {
          ...
          try {
            // Use the hook to claim evoucher
            const result = await claimEvoucher((evoucherInfo as any).claimUrl, message.id);

            if (result.success) {
              setClaimed(prev => ({ ...prev, [message.id ?? '']: true }));
              showDialog(result.message || '🎉 คุณได้รับ E-Voucher แล้ว!', 'success');
            } else {
              // ตรวจสอบว่าเป็นกรณีที่เคยรับไปแล้วหรือไม่
              if (result.alreadyClaimed) {
                setClaimed(prev => ({ ...prev, [message.id ?? '']: true })); // ตั้งสถานะว่ารับไปแล้ว
                showDialog('You already claimed this evoucher', 'error');
              } else {
                showDialog(result.message || 'ไม่สามารถรับ E-Voucher ได้', 'error');
              }
            }
          } catch (error) {
            ...
            <div className="p-5">
              ...
              {!isClaimed && (
                <button
                  ...
                  className={`w-full py-2.5 rounded-lg font-bold transition-all duration-300 ${isClaiming
  ...
  }, [message, claimed, claiming, handleOpenImagePreview, handleUnsend, stickerImageUrl, getMessageTextStyle, renderWithMentions, currentUsername, stickers, getStickerImageUrl, showDialog, claimEvoucher]);
  ...
  const renderReplyPreview = useCallback(() => {
    ...
    return (
      ...
      >
        <div
          className={`max-w-[85%] bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl p-3 border ${isMyMessage
        ...
        >
          <div className="flex items-center text-xs font-medium mb-1.5">
            <svg
              className={`w-3.5 h-3.5 mr-1.5 flex-shrink-0 ${isMyMessage ? 'text-blue-500' : 'text-gray-500 dark:text-gray-400'
                }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            ...
            <span className={`truncate ${isMyMessage
          ...
          <div className={`text-sm truncate pl-4 border-l-2 ${isMyMessage
            ...
            {enrichedReplyTo.notFound ? (
              ...
              <div className="space-y-1.5">
                {enrichedReplyTo.type === 'file' && enrichedReplyTo.fileType?.startsWith('image/') ? (
                  <div className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-4 h-4 mr-2 ${isMyMessage ? 'text-blue-400' : 'text-gray-400'
                      }`}>
                  ...
                  <div className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-4 h-4 mr-2 ${isMyMessage ? 'text-amber-400' : 'text-amber-500'
                      }`}>
                      🎁
                    </span>
                    <span className="text-sm truncate">
                  ...
                  <div className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-4 h-4 mr-2 ${isMyMessage ? 'text-purple-400' : 'text-purple-500'
                      }`}>
                      😀
                    </span>
                    <span className="text-sm truncate">
    ...
    <>
      ...
      {showClaimDialog && (
        <div className="fixed inset-0 flex items-center justify-center z-[9999] bg-black/50 backdrop-blur-sm" onClick={(e) => {
          ...
          <div className={`max-w-sm w-full mx-4 p-6 rounded-2xl shadow-2xl transform transition-all duration-300 ${claimDialogType === 'success'
            ...
            <div className="text-center">
              <div className={`text-4xl mb-4 ${claimDialogType === 'success' ? 'animate-bounce' : ''
                }`}>
                {claimDialogType === 'success' ? '🎉' : claimDialogType === 'error' ? '❌' : 'ℹ️'}
              </div>
              <h3 className={`text-lg font-bold mb-2 ${claimDialogType === 'success' ? 'text-green-800'
              ...
              <p className={`text-sm leading-relaxed ${claimDialogType === 'success' ? 'text-green-700'
              ...
              <button
                ...
                className={`mt-4 px-6 py-2 rounded-xl font-medium transition-all duration-200 ${claimDialogType === 'success'
