...
// เพิ่ม import สำหรับ QR Scanner
import dynamic from 'next/dynamic';
import { apiRequest } from '@/utils/api';
import { useRouter } from 'next/navigation';
// import { usePopupDialog } from '@/components/ui/dialog'; // ลบออก
import { CustomFinderTracker } from './_components/CustomFinderTracker';

// ใช้ dynamic import เพื่อหลีกเลี่ยง SSR error
...
export default function QRCodePage() {
  ...
  // const { openDialog } = usePopupDialog(); // ลบออก
  ...
  function showErrorAlert(message: string) {
    // ถ้า error มีคำว่า internal server ให้แสดงข้อความใหม่
  ...
  const handleBarcodeScanned = async (data: string) => {
    ...
    try {
      ...
      // กรณีอื่นๆ
      if (res.statusCode !== 200 && res.statusCode !== 201) {
        // ดัก error ที่มี internal server
        showErrorAlert(res.message || 'Check in failed');
        return;
      }
      addToast({
