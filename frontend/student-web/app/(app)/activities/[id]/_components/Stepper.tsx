...
export default function Stepper({
    ...
    return (
        <div className={`flex ${direction === "horizontal" ? "flex-row" : "flex-col"} relative`}>
            {steps.map((step, i) => {
                const stepIndex = i + 1;
                ...
                return (
                    <div
                        key={i}
                        className={`relative flex ${direction === "horizontal"
            ...
            `}
                        ...
                        {/* Step Label */}
                        <div className={`${direction === "horizontal" ? "mt-2 text-center" : "ml-4 flex flex-col"}`}>
                            <span className="font-bold text-sm">{step.title}</span>
                            {step.value && <span className="text-xs text-gray-500">{step.value}</span>}
                        </div>
                    </div>
                );
            })}
