...
export function useActivities(activityId: string | null) {
    ...
    const [answers, setAnswers] = useState<{ assessment: string; answer: string }[]>([]);
    ...
    const fetchAssessmentByActivity = async (id: string) => {
        ...
        setError(null);
        try {
            const res = await apiRequest<{ data: Assessment[] }>(`/activities/${id}/assessment`, 'GET');
            ...
            setError(
                err && typeof err === 'object' && 'message' in err
                    ? (err as { message?: string }).message || 'Failed to fetch assessments by activity.'
    ...
    const createAssessmentAnswers = async (answerData: Partial<AssessmentAnswer>) => {
        try {
            setLoading(true);
            const res = await apiRequest<AssessmentAnswer>("/assessment-answers", "POST", answerData);
    ...
    return { activities, assessments, assessmentAnswers, answers, setAnswers, loading, error, fetchActivitiesByUser, fetchAssessmentByActivity, createAssessmentAnswers };
}
