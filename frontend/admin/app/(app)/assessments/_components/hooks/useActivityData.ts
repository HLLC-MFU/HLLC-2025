...
async function fetchWithCache<T>(endpoint: string): Promise<{ data: T; message?: string }> {
    const now = Date.now();
...
function calculateStudentDetails(
    ...
    answers.forEach(answer => {
        const questionLabel = questionNameMap.get(answer.questionId) || `Question ${answer.questionId}`;
        const existing = studentMap.get(answer.userId);

        if (existing) {
            existing.completedQuestions++;
            existing.totalQuestions++;
            existing.score = Math.round((existing.score + Number(answer.answer)) / 2);
            existing.skillRatings = {
                ...existing.skillRatings,
                [questionLabel]: Number(answer.answer)
            };
        } else {
...
export function useActivityData() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError(null);
            try {
                const [answersResult, questionsResult, summaryResult] = await Promise.all([
                    apiRequest<AssessmentAnswerResponse>('/assessment-answers'),
                    apiRequest<{ data: Question[] }>('/prepost-questions'),
                    fetchWithCache<QuestionSummaryResponse[]>('/assessment-answers/all/average')
                ]);
