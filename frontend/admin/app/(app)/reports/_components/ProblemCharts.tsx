...
const CHART_COLORS = [COLORS.danger, COLORS.warning, COLORS.success];

const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
        <p className="font-medium text-black dark:text-black">{label}</p>
        <p className="text-sm text-neutral-800 dark:text-neutral-800">
...
export function ProblemCharts({ problems, reporttypes }: ProblemChartsProps) {
  ...
  return (
    <Card className="bg mb-6">
      <CardBody>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h3 className="text-lg font-medium">Reports by Category</h3>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium">Reports by Status</h3>
            <div className="h-[250px]">
              <ResponsiveContainer height="100%" width="100%">
                <PieChart>
                  <Pie
                        ...
                        if (viewBox && 'cx' in viewBox && 'cy' in viewBox) {
                          return (
                            ...
                            >
                              ...
                              <tspan
                                className="fill-back dark:fill-white text-sm"
                                x={viewBox.cx}
                                y={(viewBox.cy || 0) + 24}
                              >
                                Resolution Rate
      ...
      <CardFooter className="flex-col gap-2 text-sm">
        <div className="flex items-center gap-2 font-medium leading-none">
          {totalProblems > 0 && (
            <>
              Active reports: {totalProblems} <TrendingUp className="h-4 w-4" />
            </>
          )}
        </div>
        <div className="leading-none text-muted-foreground">
          Showing real-time report statistics
        </div>
      </CardFooter>
    </Card>
  );
