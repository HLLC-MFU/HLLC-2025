...
import { Badge, Button, Input, Select, SelectItem, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter } from "@heroui/react";
import { Users, UserPlus, X } from "lucide-react";
import { useUsers } from "@/hooks/useUsers";

interface RoomMembersSelectorProps {
...
export function RoomMembersSelector({ selectedMembers, setSelectedMembers }: RoomMembersSelectorProps) {
    ...
    return (
        <div className="space-y-4">
                        ...
                        {getSelectedUserNames().map((username, index) => (
                            <div key={selectedMembers[index] || index} className="flex items-center gap-1">
                                ...
                                <Button
                                    ...
                                    variant="light"
                                    color="danger"
                                    onPress={() => handleUserSelect(selectedMembers[index])}
                                >
                                    <X size={12} />
                                </Button>
                            </div>
            ...
            {/* Search Results */}
            {searchQuery && users.length > 0 && !isSelectAllActive && (
                <div className="space-y-2">
            ...
            {searchQuery && !usersLoading && users.length === 0 && !isSelectAllActive && (
            ...
            <Modal isOpen={showSelectAllModal} onClose={() => setShowSelectAllModal(false)} size="sm">
                <ModalContent>
                    ...
                    <ModalFooter>
                        <Button variant="light" onPress={() => setShowSelectAllModal(false)}>
                            Cancel
                        </Button>
                        <Button color="secondary" onPress={confirmSelectAll}>
                            Confirm
                        </Button>
                    </ModalFooter>
                </ModalContent>
